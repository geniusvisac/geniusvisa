  ...
  return (
    <section id="precios" className="bg-[#F8F6F1] py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-5 md:px-8">
          ...
          {plans.map((plan, i) => (
            <Reveal key={plan.name} delay={i * 100}>
              <div className={`rounded-2xl p-8 h-full flex flex-col ${plan.highlighted ? 'bg-[#1A3A3A] shadow-2xl ring-2 ring-[#C9A84C]' : 'bg-white shadow-md border border-[#1A3A3A]/5'}`}>
                ...
                <p className={`text-sm mb-6 ${plan.highlighted ? 'text-white/60' : 'text-[#1A3A3A]/60'}`}>{plan.description}</p>
                <ul className="space-y-3 flex-1 mb-8">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-2">
                      <span className="text-[#C9A84C] mt-0.5 text-sm">✓</span>
                      <span className={`text-sm ${plan.highlighted ? 'text-white/75' : 'text-[#1A3A3A]/70'}`}>{feature}</span>
                    </li>
                  ))}
                </ul>
                <a href={plan.href} target="_blank" rel="noopener noreferrer" className={`w-full text-center py-3 rounded-full text-sm font-bold transition-all hover:opacity-90 ${plan.highlighted ? 'bg-[#C9A84C] text-[#1A3A3A]' : 'border-2 border-[#1A3A3A] text-[#1A3A3A] hover:bg-[#1A3A3A] hover:text-white'}`}>
                  {plan.cta} →
                </a>
              </div>
            </Reveal>
        ...
        <Reveal>
          <p className="text-center text-[#1A3A3A]/40 text-xs mt-10">
